...
class LEVIRCCDataset_pre(Dataset):
    """
    ...
    def __getitem__(self, index):
        ...
        if datafiles["token"] is not None:
            ...
            # print(caption_list,type(caption_list[0]))
            

            # 调用函数并传入文件路径
            file_path = datafiles["token_pre"] # 这里替换为你的文件路径
            caption_list_pre = process_text(file_path)
            # print(caption_list_pre)
            ...
            tokens_encode_pre = encode(caption_list_pre, self.word_vocab,
                                    allow_unk=self.allow_unk == 1)
        else:
        ...
        return imgA.copy(), imgB.copy(), token_all.copy(), token_all_len.copy(), token.copy(), np.array(token_len), name,tokens_encode_pre
...
if __name__ == '__main__':
    ...
    train_dataset = LEVIRCCDataset(data_folder='/data/lky/data/ChangeCaption/LEVIR-CC/images',list_path='./data/LEVIR_CC1/', split= 'train', token_folder=None)
